...
class ErrorDetector:
    ...
    def _analyze_single_decision(self, row):
        ...
        # 2. Check Q_values inconsistency
    ...
    def _check_q_value_consistency(self, row):
        ...
        if abs(component_sum - total_delta) > 0.01:  # Allow small margin of error
        ...
        for component in ['turn', 'goal', 'blocked', 'safe']:
            ...
            if component in row['MSX+']:
                if component == 'goal' and delta > 0:
                    ...
                    positive_factors.append(f"Safer ({delta:+.2f})")
                elif component == 'blocked' and delta > 0:
                    positive_factors.append(f"Better avoid blocking ({delta:+.2f})")
...
class AutoValidationSystem:
    ...
    def generate_report(self):
        ...
        print(f"Report created: {self.config.OUTPUT_FILE}")
        self._print_summary()
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print(f"Results saved in: {system.config.OUTPUT_FILE}")
    else:
