...
class RewardPathFinder:
    def __init__(self, grid_size, maze_file):
        ...
        self.final_steps = 0
        self.final_turns = 0
        self.episodes = 0  # trong __init__
    ...
    def choose_action(self, state):
        ...
        if not valid_actions:
            return random.randint(0, 3)  # fallback nếu bị kẹt

        # Exploration: chọn ngẫu nhiên với xác suất epsilon
        if random.uniform(0, 1) < self.epsilon:
            return random.choice(valid_actions)

        # Tính tổng Q từ các reward components
        total_q = np.sum(self.q_table[:, x, y, :], axis=0)

        # Lấy Q-value tương ứng các hành động hợp lệ
        q_values = np.array([total_q[a] for a in valid_actions])

        # Nếu tất cả Q đều âm, shift để đảm bảo không âm
        min_q = np.min(q_values)
        if min_q < 0:
            q_values = q_values - min_q

        # Nếu tổng Q = 0 (tránh chia 0), fallback random
        f_sum = np.sum(q_values)
        if f_sum == 0:
            return random.choice(valid_actions)

        # Tính xác suất phân phối: Q_i / tổng Q
        probs = q_values / f_sum

        # Chọn hành động theo phân phối Roulette Wheel
            ...
            if episode == episodes - 1 and log_random_episode:
                ...
                for i in range(self.grid_size):
                    for j in range(self.grid_size):
                        ...
                        gamma_multiplier = 1.0  # Không dùng gamma_dict nữa


                        # Tính xác suất roulette cho từng component
                ...
                # Ghi xác suất roulette cho mỗi state
                roulette_rows = []
                for i in range(self.grid_size):
                    for j in range(self.grid_size):
                        state = (i, j)
    ...
    def get_roulette_probabilities(self, state):
        ...
        for c_idx, component in enumerate(self.reward_components + ['total']):
            ...
            # Shift về dương để tránh số âm
            q_shifted = q_values - np.min(q_values) + 1e-6
            probs = q_shifted / np.sum(q_shifted)
            
            probs_dict[component] = probs  # 4 giá trị ứng với [up, down, left, right]
